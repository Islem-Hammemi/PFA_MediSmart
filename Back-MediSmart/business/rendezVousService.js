...
const _formaterRdvMedecin = (row) => ({
  rdv_id:               row.rdv_id,
  date_heure:           row.date_heure,
  date_heure_formatee:  row.date_heure_formatee || null,
  date:                 row.date  || null,
...
const reserverViaCreneau = async (userId, { medecinId, creneauId, motif }) => {
  ...
  try {
    ...
    if (conflit) throw new Error("Conflit détecté.");
...
// ── ✅ NOUVEAU : pending requests (statut = planifie) ─────────
...
// ── ✅ NOUVEAU : changer statut (confirmer / refuser / terminer)
const changerStatut = async (rdvId, medecinId, nouveauStatut, statutsAutorisés) => {
  ...
  if (!affected) throw new Error("RDV introuvable ou action non autorisée.");

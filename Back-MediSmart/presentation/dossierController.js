...
const getDossier = async (req, res) => {
  try {
    // Récupérer le patient_id depuis la table patients via user_id
    ...
    const dossiers  = await dossierRepository.getByPatientId(patientId);
...
const getDossiersPatient = async (req, res) => {
  try {
    ...
    if (!patientId || isNaN(patientId)) {
      return res.status(400).json({ success: false, message: 'patientId invalide.' });
    }
    const aUneLiaison = await dossierRepository.verifierRelation(medecinId, patientId);
    if (!aUneLiaison) {
